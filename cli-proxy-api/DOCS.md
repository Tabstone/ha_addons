...
首次启动后会在 `/addon_configs/9d486770_cli-proxy-api/` 下自动创建如下结构：

```
/addon_configs/9d486770_cli-proxy-api/
  config/
    config.yaml        # 主配置文件
  auth-dir/            # 认证数据目录（OAuth token 等）
```

Home Assistant 会在 add-on slug 前加仓库前缀（例如 `9d486770_`）。如果你的安装使用了不同前缀，请把上面的路径替换为实际 slug。
...
**后台管理密钥默认值：`123456`**（请在首次启动后尽快修改）。

## 端口与 Ingress

- **Ingress**：使用 Home Assistant 侧边栏进入（入口为 `/`）
- **外部端口**：默认映射 `8317/tcp`，可在 add-on 网络设置中修改
- **管理面板路径**：`http://homeassistant.local:8317/management.html`
