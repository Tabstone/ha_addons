...
首次启动后会在 `/addon_configs/<实际 add-on slug>/` 下自动创建如下结构：
...
Home Assistant 会在 add-on slug 前加仓库前缀（例如 `9d486770_`）。请将 `<实际 add-on slug>` 替换为 Home Assistant add-on 列表中显示的完整 slug。
